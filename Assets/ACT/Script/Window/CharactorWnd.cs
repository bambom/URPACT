...
//		Control ("AbUseAdd").GetComponent<UILabel>().text =      "AbUseAdd:"+Attrib.AbHitAdd.ToString();		
	...
	void OnUnEquSuccess()
	{
		//NGUIDebug.Log("OnUn=====EquSuccess!!");
		RequestPackageInfo();
	}
	
	void OnEquSuccess()
	{
		//NGUIDebug.Log("OnEquSuccess!!");
		RequestPackageInfo();
	}
	...
	void OnClickEquip( GameObject go )
	{
	    ...
	    Request(new EquipCmd(nIndex), delegate(string err, Response response)
        {
	...
	void OnClickUnEquip( GameObject go )
	{
	    ...
	    Request(new UnEquipCmd(nIndex), delegate(string err, Response response)
        {
            if (!string.IsNullOrEmpty(err))
            {
				MessageBoxWnd.Instance.Show( err ,MessageBoxWnd.StyleExt.Alpha );
